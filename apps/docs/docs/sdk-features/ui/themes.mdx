...
Themes restyle the **chrome only** — toolbar, panels, menus, popovers. The canvas stays light in every theme: element colors are plain hex values stored in the scene, authored against light paper, so a dark canvas would silently break user content. Dark mode = dark chrome around a light drawing.
