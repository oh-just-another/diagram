...
Board › Background color lists `CANVAS_BACKGROUND_PRESETS` (`@oh-just-another/editor`): White, Light gray (default), Gray, Dark gray, Black, Light blue. The `<Editor>` root's `--du-canvas-bg` follows the scene, so a document looks the same in every host; without a colour in the document the stylesheet token stays in charge. The dark chrome theme never recolours the paper.
