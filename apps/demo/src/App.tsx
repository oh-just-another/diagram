...
import type { TextShaper } from "@oh-just-another/renderer-core";
import { readInitialWasmText, persistWasmText } from "./wasm-text-mode";
...
export const App = () => {
 ...
 // WASM text shaping toggle. URL ?wasm=1 OR ?wasm=text
 // turns it on at boot; localStorage persists the choice. The
 // shaper is loaded async — until it's ready DiagramRoot mounts
 // with the default Canvas2D measure path; on resolve we re-render
 // with the WASM shaper, which forces DiagramRoot to re-create the
 // editor and pick up the textShaper prop.
 const [wasmText, setWasmText] = useState<boolean>(() => readInitialWasmText());
 const [wasmShaper, setWasmShaper] = useState<TextShaper | null>(null);
 const wasmStatus = wasmText
  ? wasmShaper
   ? "wasm-ready"
   : "wasm-loading"
  : "off";
 useEffect(() => {
  ...
  let cancelled = false;
  void WasmTextShaper.loadBundled().then(
   (s) => {
    if (!cancelled) setWasmShaper(s);
   },
   (err) => {
    console.warn("[demo] WASM text shaper failed to load, staying on Canvas2D", err);
    if (!cancelled) setWasmText(false);
   },
  );
  return () => {
   cancelled = true;
 ...
 const handleWasmTextToggle = useCallback((next: boolean) => {
  setWasmText(next);
  persistWasmText(next);
 }, []);
 useHotkeys(editor);
  ...
  <div className="root" style={{ display: "flex", flexDirection: "column", height: "100%" }}>
   ...
   >
    <div style={{ display: "flex", alignItems: "center", gap: 16 }}>
     ...
     <label
      title={
       wasmStatus === "wasm-ready"
        ? "Text wrap measurements use the bundled Roboto Regular WASM shaper (deterministic Node vs browser)"
        : wasmStatus === "wasm-loading"
         ? "Loading WASM text shaper..."
         : "Click to enable WASM text shaping (Roboto Regular bundled, ~590 KB)"
      }
      style={{
       display: "inline-flex",
       alignItems: "center",
     ...
     >
      <input
       type="checkbox"
       checked={wasmText}
       onChange={(ev) => handleWasmTextToggle(ev.target.checked)}
      />
      <span style={{ textTransform: "uppercase", letterSpacing: 0.5 }}>
       wasm text
      </span>
      {wasmStatus === "wasm-loading" ? <span>…</span> : null}
      {wasmStatus === "wasm-ready" ? <span style={{ color: "var(--accent)" }}>✓</span> : null}
     </label>
     {room ? (
      <span
       style={{
   ...
   <DiagramRoot
    key={`${rendererMode}|${wasmShaper ? "wasm" : "canvas"}`}
