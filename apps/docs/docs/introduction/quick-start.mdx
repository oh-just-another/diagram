...
Props such as `initialMode`, `theme`, `hideToolbar`, and the `renderTopBar*` slots let you toggle or replace parts of the UI, and `onSceneChange` / `onSelectionChange` notify you of edits. See the full props reference in the [package README](https://github.com/oh-just-another/diagram/tree/master/packages/editor).
