...
| `getActiveTool()` / `setActiveTool(t)` | `ActiveTool` / `Mode`       | Read the active-tool object / switch the tool.               |
...
| `renderTopBar{Left,Center,Right}` / `renderBottomBar{Left,Center,Right}` / `renderMainMenuExtras` / `renderBoardMenuExtras`                                                                     | `() => ReactNode`     | Inject custom content into the bars / menu.                                             |
