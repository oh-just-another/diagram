...
Pass a `ref` to get an `EditorAPI` handle. It exposes curated verbs — `getScene`, `loadScene`, `getActiveTool` / `setActiveTool`, `getSelection` / `setSelection`, `undo` / `redo`, `zoomToFit` — plus `editor`, the full live engine (`EditorInstance`) as the escape hatch (`null` until `onReady` fires).
