...
`@oh-just-another/importers` converts popular diagram source formats — Mermaid flowcharts, Graphviz dot, drawio XML, `.excalidraw` JSON, and JSON Canvas — into a `Scene`. It's the bridge for text-authored or LLM-generated diagrams. This page covers Mermaid; see [Importers](./importers.mdx) for the full format matrix.
