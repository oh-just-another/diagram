...
`@oh-just-another/diagram-svelte` wraps the [`<oja-diagram>` custom element](/docs/examples/custom-element) as a Svelte 5 component (runes; ships `.svelte` source compiled by your bundler). Svelte can't run inside this React docs site — see the [React pages](/docs/examples/react-basic) for live demos.
