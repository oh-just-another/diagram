...
`EditorAPI` covers scene (`getScene` / `loadScene`), tool (`getActiveTool` / `setActiveTool`), selection (`getSelection` / `setSelection`), history (`undo` / `redo`) and viewport (`zoomToFit`).
...
Because the engine is the single source of truth, programmatic control and human interaction stay in sync — a script that calls `setActiveTool` or `addElement` produces exactly the same scene a click would. This is the foundation the AI-native direction builds on: an agent drives the editor through the very same `EditorAPI` / `EditorInstance` surface documented here — see [AI](./ai.mdx).
