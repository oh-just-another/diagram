...
`EditorAPI` exposes the common operations without reaching into internals. The handle is live the moment the editor mounts (`onReady`); `editor` and `capabilities` are `null` before then.
