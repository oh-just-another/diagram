...
const recordingTarget = () => {
  ...
  const t = {
    ...
    setLineCap: () => {},
    setLineJoin: () => {},
    setDashArray: (d: readonly number[] | null) => {
      dashed = d !== null && d.length > 0;
    },
    setFont: () => {},
    setTextAlign: () => {},
    setTextBaseline: () => {},
...
describe("draw-by-drag shows a WYSIWYG shape preview (not just a dashed box)", () => {
  it("draw-ellipse drag renders the actual ellipse during the gesture", () => {
    ...
    const editor = new Editor({
      host: host(handlers),
      mainTarget: recordingTarget().target,
    ...
    expect(overlay.ellipses.some((e) => Math.abs(e.rx - 25) < 2 && Math.abs(e.ry - 20) < 2)).toBe(
      true,
    );
