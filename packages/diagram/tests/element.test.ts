...
import { OJA_DIAGRAM_TAG, OjaDiagramElement, defineOjaDiagram } from "../src/index";

// These exercise the element's DOM contract without connecting it to a live
// document, so no React render / WASM load happens.
describe("<oja-diagram> custom element", () => {
  it("registers the tag on import", () => {
    expect(customElements.get(OJA_DIAGRAM_TAG)).toBe(OjaDiagramElement);
  });

  it("defineOjaDiagram is idempotent", () => {
    expect(() => {
      defineOjaDiagram();
    }).not.toThrow();
  });
  ...
  it("observes the configuration attributes", () => {
    expect([...OjaDiagramElement.observedAttributes]).toEqual([
      "theme",
      "renderer",
      "grid",
      "snap",
