<script lang="ts">
  // Side effect: registers the <oh-diagram> custom element on import, so a
  // Svelte host never has to call `defineOhDiagram`.
  ...
  interface Props {
    ...
    grid?: boolean;
    snap?: boolean;
    onready?: (detail: OhDiagramEventMap["ready"]) => void;
    onscenechange?: (detail: OhDiagramEventMap["scenechange"]) => void;
    onselectionchange?: (detail: OhDiagramEventMap["selectionchange"]) => void;
    onthemechange?: (detail: OhDiagramEventMap["themechange"]) => void;
  }
  ...
  let el = $state<OhDiagramElement>();

  // Push declarative props onto the element whenever any of them change.
  // `applyOhDiagramProps` is idempotent, so a single effect covers them all.
  $effect(() => {
    if (el) applyOhDiagramProps(el, { scene, theme, renderer, grid, snap });
  });
  ...
  const api = ohDiagramController(() => el ?? null);
...
<oh-diagram bind:this={el} style="display:block;width:100%;height:100%"></oh-diagram>
