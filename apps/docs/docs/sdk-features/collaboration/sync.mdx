...
- `WebSocketTransport` — syncs across machines through a relay server, with a `WebSocketStatus` you can surface in the UI and `WebSocketTransportOptions` for configuration.
...
const remote = new WebSocketTransport({ url: "wss://example.com/room-foo" } as never);
```
