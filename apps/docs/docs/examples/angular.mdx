...
`@oh-just-another/diagram-angular` wraps the [`<oja-diagram>` custom element](/docs/examples/custom-element) as a standalone Angular component, `<oja-diagram-ng>`. Angular can't run inside this React docs site — see the [React pages](/docs/examples/react-basic) for live demos.
