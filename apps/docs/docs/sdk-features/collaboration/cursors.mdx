...
`CollabAwareness` exposes the current peers and a subscription for changes, which you can use to render an avatar stack or a "who's here" list.

```ts
import type { Peer } from "@oh-just-another/collab";

const peers: Peer[] = awareness.getOthers();
