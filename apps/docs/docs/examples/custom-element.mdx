...
`@oh-just-another/diagram` ships the editor as a `<oja-diagram>` custom element — React is bundled inside, styles are isolated in a shadow root, and the whole surface is plain DOM. This page can't run it live (the docs site is React); see [React: drop-in](/docs/examples/react-basic) for a live demo.
...
<html>
  <head>
    ...
    <style>
      body { margin: 0 }
      oja-diagram { display: block; height: 100vh }
    </style>
  </head>
  <body>
...
- **Attributes:** `theme` (`dark` | `light` | `system`), `renderer` (`canvas2d` | `webgl2` | `offscreen`, omit to auto-detect), `grid` / `snap` (boolean — attribute present = on).
