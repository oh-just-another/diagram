...
- **Pressure** — pens use the device pressure channel (rate-limited against outlier samples); mouse and touch simulate it from pointer speed: slow is thick, fast is thin.
...
- **Smoothing and taper** — on commit the polyline is resampled along a Catmull-Rom spline and the ends taper down like a lifting pen (closed, filled loops keep their full-width seam).
