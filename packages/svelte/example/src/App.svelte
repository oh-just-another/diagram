<script lang="ts">
  ...
  import OhDiagram from "../../src/OhDiagram.svelte";
  import {
    parseScene,
    parseFiles,
    stringifyScene,
  ...
  // localStorage round-trip through the component's events so a reload keeps
  // the scene (and its image / GIF bytes).
  const SCENE_KEY = "oh-svelte-example-scene";
  const FILES_KEY = "oh-svelte-example-files";
...
<div style="height:100vh">
  <OhDiagram {scene} theme="system" grid snap {onscenechange} />
</div>
