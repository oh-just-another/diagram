...
Pick a `@oh-just-another/network` transport for your topology: `BroadcastChannelTransport` for tabs on the same origin, `WebSocketTransport` for cross-machine rooms (with auto-reconnect). Any transport implementing the `Transport` interface works.
