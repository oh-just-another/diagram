...
`LodOptions` lets cheaper paths kick in at low zoom: below `placeholder` zoom,
shapes degrade to flat AABB fills; below `hideText` zoom, text layout and
glyph drawing are skipped. Pass it via `RenderSceneOptions.lod`; the defaults
are exported as `DEFAULT_LOD` (`placeholder: 0.15`, `hideText: 0.4`).
