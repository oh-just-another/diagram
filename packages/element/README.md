# @oh-just-another/element

The diagram editor as a framework-neutral custom element — `<oh-diagram>`.
...
```bash
pnpm add @oh-just-another/element
```
...
```html
<oh-diagram grid theme="dark" style="height: 100vh"></oh-diagram>
```
...
<script
  type="module"
  src="https://unpkg.com/@oh-just-another/element/dist/oh-diagram.global.js"
></script>
<oh-diagram grid style="height: 100vh"></oh-diagram>
```
...
`defineOhDiagram(tag?)` registers the element (the package does this on import); pass a tag name to register under a different name.
