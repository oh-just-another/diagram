...
describe("patch", () => {
  describe("apply", () => {
    it("add: before=null, after=shape", () => {
      ...
      expect(scene.elements.get(shape.id)).toBe(shape);
    });
    it("remove: before=shape, after=null", () => {
      ...
      expect(s2.elements.has(shape.id)).toBe(false);
    });
    it("update: before=A, after=B", () => {
      ...
      expect(s2.elements.get(shape.id)?.position).toEqual({ x: 5, y: 5 });
    });
  });
  ...
  describe("invert", () => {
    ...
    it("applying patch then its inverse returns the original scene", () => {
      ...
      expect(end.elements.size).toBe(0);
    });
  ...
  describe("batch", () => {
    ...
    it("apply runs patches in order", () => {
      ...
      expect(final.elements.size).toBe(2);
    });
  });
