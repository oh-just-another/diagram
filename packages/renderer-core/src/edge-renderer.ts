...
const drawLink = (
  ...
  if ((edge.lineKind ?? "line") === "block-arrow") {
    ...
    if (curve) {
      ...
      // LINK_CORNER_RADIUS is a tunable constant; guard stays valid if retuned to 0.
...
const drawBlockArrowLink = (edge: Link, path: readonly Vec2[], target: RenderTarget): void => {
  ...
  // to build the body polygon (manual one-segment offset; mitre
  // joins on orthogonal routes look fine).
  const left = offsetPolyline(shortened, thickness / 2);
  const right = offsetPolyline(shortened, -thickness / 2);
  const headBaseLeft = req(left[left.length - 1]);
  const headBaseRight = req(right[right.length - 1]);
  const tip = req(path[path.length - 1]);
  ...
  const leftFirst = req(left[0]);
  target.moveTo(leftFirst.x, leftFirst.y);
  for (let i = 1; i < left.length; i++) {
    const p = req(left[i]);
    target.lineTo(p.x, p.y);
  }
  // Head triangle: head base (left) → tip → head base (right).
  // The "barbs" make the head wider than the body so it reads as a
  // proper block-arrow head, not just a continuation.
...
const shortenPathFromEnd = (path: readonly Vec2[], amount: number): readonly Vec2[] => {
  ...
  const out: Vec2[] = [req(reversed[0])];
  for (let i = 1; i < reversed.length; i++) {
    const a = req(out[out.length - 1]);
    const b = req(reversed[i]);
    const dx = b.x - a.x;
    const dy = b.y - a.y;
...
/**
 * Per-vertex perpendicular offset of a polyline. Works well for
 * orthogonal / straight routing; bezier-routed paths get piecewise
 * approximations.
 */
const offsetPolyline = (path: readonly Vec2[], offset: number): readonly Vec2[] => {
  ...
  for (let i = 0; i < path.length; i++) {
    const prev = req(path[Math.max(0, i - 1)]);
    const next = req(path[Math.min(path.length - 1, i + 1)]);
...
const cornersOnly = (pts: readonly Vec2[]): readonly Vec2[] => {
  ...
  const out: Vec2[] = [req(pts[0])];
  for (let i = 1; i < pts.length - 1; i++) {
    const prev = req(out[out.length - 1]);
    const cur = req(pts[i]);
    const next = req(pts[i + 1]);
...
export const strokeRoundedPolyline = (
  ...
  const ptsLast = req(pts[pts.length - 1]);
  target.lineTo(ptsLast.x, ptsLast.y);
};
