...
Separate from element copy/paste, the engine keeps a lightweight per-editor _style_ buffer — capture a shape's visual style (fill, stroke, dash, …) and stamp it onto the current selection.
