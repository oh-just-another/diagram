...
The transport uses the platform's native `BroadcastChannel` — no polyfill or extra dependency.
...
`waitForSyncMs` makes a newly opened tab wait briefly for the room's state before seeding the doc from its own scene — without it (default 0) a joiner with a non-empty editor can clobber the shared document.
