...
import "../src/index"; // side effect: registers <oh-diagram>
import {
  applyOhDiagramProps,
  bindOhDiagramEvents,
  OH_DIAGRAM_EVENTS,
...
import type { OhDiagramElement } from "../src/oh-diagram-element";

// A bare `<oh-diagram>` is never connected here, so no React / WASM mount
// happens — these exercise the pure attribute / property / event plumbing.
const makeEl = (): OhDiagramElement => document.createElement("oh-diagram");

describe("applyOhDiagramProps", () => {
  it("maps scalars to attributes and the scene to the property", () => {
    const el = makeEl();
    const scene = { schemaVersion: 1 } as unknown as Scene;
    applyOhDiagramProps(el, { theme: "dark", renderer: "webgl2", grid: true, snap: true, scene });
  ...
  it("clears attributes when a prop is absent (wrapper owns the element)", () => {
    const el = makeEl();
    applyOhDiagramProps(el, { theme: "light", grid: true });
    applyOhDiagramProps(el, {});
...
describe("ohDiagramController", () => {
  it("delegates to the current element and stays inert when absent", () => {
    let el: OhDiagramElement | null = null;
    const controller = ohDiagramController(() => el);
