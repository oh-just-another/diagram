...
A press becomes a _drag_ only once the pointer travels `DRAG_THRESHOLD` from the press origin; below that it resolves to a _click_. The pure helper `interpretPressEnd` makes that decision, and handle hit zones stay a fixed CSS size at any zoom because tolerance is divided by viewport zoom.
