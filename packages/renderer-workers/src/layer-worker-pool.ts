 ...
 *
 * Each layer owns its own OffscreenCanvas (transferred once during
 * `init`), so layer L must always go back to the same worker that
 * holds its canvas.
 *
...
export class LayerWorkerPool {
  ...
  submitForLayer<T>(layerId: LayerId, fn: (worker: Worker) => Promise<T>): Promise<T> {
    ...
    // Bypass the round-robin pool — target this specific worker.
    return fn(worker);
  }
