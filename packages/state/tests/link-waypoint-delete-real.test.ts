...
const scene = (): Scene => {
  ...
  const link: Link = {
    id: linkId("L"), layerId: DEFAULT_LAYER_ID, order: orderBetween(null, null),
    ...
    style: { stroke: "#000" }, routing: "orthogonal", arrowheads: { to: "triangle" },
  };
...
describe("waypoint delete on a straight (anchored) link — real flow", () => {
  it("switch to straight, add a waypoint, drop it back on the line → removed", () => {
    ...
    const editor = new Editor({ host, mainTarget: noopTarget, overlayTarget: noopTarget, initialScene: scene() });
    ...
    down(120, 100); up(120, 100); // select the link
    expect(editor.selectedLink).not.toBeNull();
    editor.updateSelectedLink((e) => ({ ...e, routing: "straight" }));

    down(120, 100); move(120, 160); up(120, 160); // add a waypoint (drag segment midpoint)
    expect((L().waypoints ?? []).length).toBe(1);

    down(120, 160); move(120, 103); up(120, 103); // drop back on the chord (y≈100) → collapse
