...
thresholds — a full repaint, so do not toggle it per frame.
...
paints — `1000 / gapMs` is the fps the user sees, including everything outside
...
every paint with the same object — subscribe from diagnostics, not from
